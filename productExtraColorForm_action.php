...
if (isset($_POST['add'])) {
    ...
    $select_product_color = $con->prepare("SELECT `proClr_id`, `proClr_colorname` FROM `xtraproductcolorform` WHERE `proClr_productId`=? and proClr_image=?");
    var_dump($select_product_color);
    $select_product_color->bind_param("ss", $add_item_id, $add_item_image);
